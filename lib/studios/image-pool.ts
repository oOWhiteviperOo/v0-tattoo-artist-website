...
export function getStudioImages(slug: string) {
  ...
  return {
    ...
    portfolio: Array.from({ length: 6 }, (_, i) =>
      PORTFOLIO_IMAGES[(h + i * 7) % PORTFOLIO_IMAGES.length]
    ),
    instagram: Array.from({ length: 4 }, (_, i) =>
      INSTAGRAM_IMAGES[(h + i * 5) % INSTAGRAM_IMAGES.length]
    ),
    about: ABOUT_IMAGES[(h + 3) % ABOUT_IMAGES.length],
    og: OG_IMAGES[(h + 1) % OG_IMAGES.length],
  }
}
